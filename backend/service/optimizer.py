...
class DiskOptimizer:
    ...
    def score_disk(self, disk: dict[str, Any], weights: dict[str, float]) -> float:
        main_stat = disk.get("main_stat") if isinstance(disk, dict) else {}
        main_score = self._stat_value(main_stat) * self._weight_for_stat(main_stat, weights) * 2.0
        sub_score = sum(
            self._stat_value(stat) * self._weight_for_stat(stat, weights)
            for stat in self._sub_stats(disk)
        )
        return round(main_score + sub_score, 4)
    ...
    def find_best_combination(
        ...
        best = self._best_combo(candidates_by_slot, weights, target_set_4, target_set_2)
        if best is None:
        ...
        return {
            ...
            "combo": [dict(disk) for disk in combo],
    ...
    def _candidates_by_slot(
        ...
        candidates_by_slot: dict[int, list[dict[str, Any]]] = {}

        for slot in range(1, 7):
            slot_candidates = [disk for disk in valid_disks if disk.get("slot") == slot]
            if not slot_candidates:
                raise ValueError(f"slot {slot} has no candidates")
    ...
    def _best_combo(
        ...
        target_set_4: str,
        target_set_2: str,
    ) -> tuple[list[dict[str, Any]], float, Counter[str], str, list[dict[str, Any]]] | None:
        ...
        for combo_tuple in product(*(candidates_by_slot[slot] for slot in range(1, 7))):
            ...
            set_counts = Counter(
                disk.get("set_name") for disk in combo if isinstance(disk.get("set_name"), str) and disk.get("set_name")
            )
            match_type = self._match_type(set_counts, target_set_4, target_set_2)
            scores = [self.score_disk(disk, weights) for disk in combo]
            total_score = round(sum(scores), 4)
            key = (MATCH_TYPE_PRIORITY[match_type], -total_score)
            if best_key is None or key < best_key:
                best_key = key
                score_breakdown = [
                    {"disk": dict(disk), "score": round(score, 4)}
